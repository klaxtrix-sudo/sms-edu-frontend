  ...
  if (error) {
    return (
      <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div>
          <h1 className="text-4xl font-black tracking-tight text-foreground uppercase">My Children</h1>
          <p className="text-muted-foreground mt-2 font-medium">Detailed profiles of your enrolled children.</p>
        </div>
      ...
      {children.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {children.map(child => {
            ...
            return (
              <Card key={child.id} className="border-border/80 bg-card text-card-foreground shadow-md hover:shadow-xl transition-all duration-300 rounded-[2rem] overflow-hidden group">
                <div className="h-32 bg-primary/5 flex items-center justify-center relative">
                  <div className="absolute inset-0 bg-gradient-to-t from-black/10 to-transparent" />
                  <Avatar className="size-24 rounded-full border-4 border-card shadow-lg absolute -bottom-12">
                ...
                <CardContent className="pt-16 pb-8 px-8 text-center space-y-4">
                  <div>
                    <h3 className="text-2xl font-black tracking-tight text-foreground leading-none mb-2">{profile?.full_name}</h3>
                  ...
                  <div className="flex flex-col gap-1 text-sm text-muted-foreground font-medium pb-4">
