...
function SubjectSearchSelect({ subjects, selectedSubjectId, onSelect, disabledSubjects }: SubjectSearchSelectProps) {
  ...
  return (
    <div className="relative w-full">
      ...
      {isOpen && (
        <div className="absolute z-[9999] top-full left-0 mt-1 w-full bg-popover border border-border rounded-xl shadow-xl max-h-[300px] overflow-y-auto custom-scrollbar p-1">
          {filtered.length === 0 ? (
...
export function ManageSubjectTeachersModal({ 
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[680px] w-[95vw] h-[85vh] max-h-[850px] flex flex-col p-0 border-none shadow-2xl bg-card rounded-[2rem] overflow-hidden">
        {/* Top Accent line */}
        ...
        <div className="flex-1 overflow-y-auto px-6 md:px-8 pb-6 space-y-4 min-h-[350px] custom-scrollbar">
          {loading ? (
